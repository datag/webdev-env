...
mysql_config: mysql_install
	mkdir -p $(PREFIX)/local/mysql/etc
	cp -vt $(PREFIX)/local/mysql/etc $(FILES)/mysql/config/my.cnf
	
	( cd $(PREFIX)/local/mysql && ./scripts/mysql_install_db --defaults-file=$(PREFIX)/local/mysql/etc/my.cnf )
	-rm -f $(PREFIX)/local/mysql/my.cnf    # template generated by mysql_install_db
